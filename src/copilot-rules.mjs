...
export function planTaskScenarios(snapshot = {}) {
  ...
  if (source.kind === "sheet") {
    ...
    offer("animation", 82, "Открыта серия изображений; её можно собрать в атлас с общим таймлайном.");
    offer("combine", 75, "Если изображения — разные предметы, соберите общий атлас с JSON координат вместо множества файлов.");
...
export function planSuggestions(snapshot = {}) {
  ...
  if (hasFrameIssue(built, /ширина силуэта|высота силуэта/)) {
    add({
      ...
      why: "Силуэты в серии отличаются по размеру — персонаж будет «дышать» при проигрывании.",
